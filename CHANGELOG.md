...
- `Webhook.secretKey` is now write-only: it always resolves to `null` on read, including in the `webhookCreate`/`webhookUpdate` mutation responses. Previously, any staff user or app with `MANAGE_APPS` could read back the plaintext HMAC signing key at any time, which allowed forging valid webhook signatures.
